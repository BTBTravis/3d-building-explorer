...
//import { OrbitControls } from 'three-orbit-controls'; // TODO: only import the part of the lib we need ex. import { Scene } from 'three'; .. should speed up load by reducing the bundeled js size.
import OrbitControls from 'three-orbit-controls'; // TODO: only import the part of the lib we need ex. import { Scene } from 'three'; .. should speed up load by reducing the bundeled js size.
...
export default function (config) {
  // TODO: default config
  let res = configValidator(config);
  if(res.hasError) {
    res.errorMessages.map(message => console.log(message));
    return false;
  }
  config = res.config;
  console.log('config: ', config);
  // define global varables
  var renderContainer = config.el;
  ...
  let materials = config.materials.reduce(function (obj, config) {
    obj[config.name] = new THREE.MeshPhongMaterial({ color: config.color, name: config.name });
  return obj;
  }, {});

  // util
  var _flattenThreeObj = threeobj =>  {
    return {
  ...
  const _init = () => {
    return new Promise((resolve, reject) => {
      ...
      views.map(function (view) {
        let initCamSettings = config.initalCameraPos;
        ...
        let camera = new THREE.PerspectiveCamera(70, aspect * config.cameraAspectFactor, 10, 10000);
      ...
      //camera.fov = 90;
      // orbit setup
      if (config.orbitMode) orbit = new OrbitControls(views[0].camera);
      // camera pos tool
      if (config.debugMode) {
      ...
      loader.load(config.modelUrl, function (result) {
        // import matrix fixes
        ...
        let yOffset = config.cameraYoffset;
      ...
      const controller = (function () {
        ...
        let moveTo = config.rooms.reduce((obj, config) => {
          console.log({"config": config});
  ...
  const _animate = () => {
    ...
    if (config.orbitMode) orbit.update();
    //views.map(function (view) {
